...
def get_llm_match(resume_text: str, jd_text: str) -> dict:
    ...
    Returns a dict:
        {
          "llm_score": int | None,        # 1-10, None if unavailable
          "justification": str,
          "key_strengths": list[str],
          "key_gaps": list[str],
          "recommendation": str,
          "available": bool,              # False if no API key / call failed
          "error": str | None,
        }
    ...
    if parsed is None:
        return {
            "llm_score": None,
            "justification": raw_text.strip()[:500] if raw_text.strip() else "(empty response from model)",
            "key_strengths": [],
            "key_gaps": [],
            "recommendation": "",
            "available": False,
            "error": f"Could not parse JSON from LLM response. Raw: {raw_text.strip()[:200]!r}",
        }
